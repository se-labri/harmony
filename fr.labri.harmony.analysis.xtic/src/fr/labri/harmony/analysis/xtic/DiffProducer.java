...
import fr.labri.gumtree.io.MappingIoUtils;
import fr.labri.gumtree.io.TreeGenerator;
import fr.labri.gumtree.matchers.heuristic.LcsMatcher;
...
public abstract class DiffProducer {
	...
	private static String computeTree(File oldFile, File newFile, boolean targetNewFile, TreeGenerator generator) {
		try {
			String xml = "";
			if (oldFile == null) {
				...
				if (dst == null)
					return "";
				if(targetNewFile) {
					LcsMatcher matcher = new LcsMatcher(src, dst);
					xml = MappingIoUtils.writeXML(dst, matcher.getMappings(), true);
				}
				else {
					LcsMatcher matcher = new LcsMatcher(dst, src);
